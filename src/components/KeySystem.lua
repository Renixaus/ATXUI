...
function KeySystem.new(Config, Filename, func, keyValidator)
	...
	local KeyDialog =
		KeyDialogInit.Create(true, "Popup", Config.Window, Config.ATXUI, Config.ATXUI.ScreenGui.KeySystem)
	...
	if Config.KeySystem.API then
		...
		for _, i in next, Config.KeySystem.API do
			local serviceDef = Config.ATXUI.Services[i.Type]
			if serviceDef then
				local args = {}
	...
	local function handleSuccess(key)
		...
		if writefile then
			...
			-- Normalize: strip trailing slashes to avoid executors rejecting makefolder/isfolder.
			while #folder > 0 and (folder:sub(-1) == "/" or folder:sub(-1) == "\\") do
				folder = folder:sub(1, -2)
			end
			-- Ensure the folder exists BEFORE writing — first-run KeySystem with "Temp"
			-- (or any user-specified folder) would otherwise fail on PC exploits.
			if folder ~= "" and isfolder and makefolder then
				local okIs, exists = pcall(isfolder, folder)
				if not (okIs and exists) then
					pcall(makefolder, folder)
				end
			end
			local path = (folder ~= "" and (folder .. "/") or "") .. Filename .. ".key"
			local okWrite, errWrite = pcall(writefile, path, tostring(key))
			if not okWrite then
				warn(
