...
function DialogModule.Create(Key, Type, Window, ATXUI, Parent)
	local Dialog = {
		UICorner = 28,
		UIPadding = 12,
	...
	function Dialog:Open()
		...
		task.spawn(function()
			...
			if not Key then
				Tween(Dialog.UIElements.FullScreen, 0.18, { BackgroundTransparency = 0.3 }):Play()
			end
			Tween(Dialog.UIElements.MainContainer, 0.18, { ImageTransparency = 0 }):Play()
			Tween(
	...
	function Dialog:Close()
		...
		Tween(Dialog.UIElements.MainContainer, 0.15, { ImageTransparency = 1 }):Play()
		Tween(
			Dialog.UIElements.MainContainer.DialogScale,
			0.15,
			{ Scale = 0.92 },
