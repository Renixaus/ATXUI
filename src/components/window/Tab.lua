...
function TabModule.New(Config, UIScale)
	local Tab = {
		...
		CustomEmptyPage = (Config.CustomEmptyPage and next(Config.CustomEmptyPage) ~= nil)
				and Config.CustomEmptyPage
			...
			local _IconBG = Creator.NewRoundFrame(
				...
				},
				{
					...
					Creator.NewRoundFrame(
						Tab.IconShape ~= "Circle" and (Tab.UICorner + 5 - (2 + (Window.UIPadding / 4)))
							or 9999,
						"Glass-1.4",
						{
							Size = UDim2.new(1, 0, 1, 0),
	...
	Tab.UIElements.ContainerFrameCanvas = New("Frame", {
		...
		New("UIScale", { Scale = 1, Name = "TabScale" }),
	...
	function Tab:GetLocked()
		...
		for _, element in next, Window.AllElements do
			if
				element.Tab
				and element.Tab.Index
				and element.Tab.Index == Tab.Index
				and element.Locked == true
			then
	...
	task.spawn(function()
		...
		if Tab.CustomEmptyPage.Icon then
			EmptyPageIcon = Creator.Image(
				Tab.CustomEmptyPage.Icon,
				Tab.CustomEmptyPage.Icon,
				0,
				"Temp",
				"EmptyPage",
	...
	if not TabModule.Tabs[TabIndex].Locked then
		...
		task.spawn(function()
			for _, ContainerObject in next, TabModule.Containers do
				ContainerObject.AnchorPoint = Vector2.new(0, 0.04)
				ContainerObject.Visible = false
				if ContainerObject:FindFirstChild("TabScale") then
					ContainerObject.TabScale.Scale = 0.97
				end
			end
			...
			if newContainer:FindFirstChild("TabScale") then
				Creator.Tween(
					newContainer.TabScale,
					0.28,
					{ Scale = 1 },
