...
return function(Config)
	...
	Window.TopBarButtonIconSize = Window.TopBarButtonIconSize
		or (Window.Topbar.ButtonsType == "Mac" and 11 or 16)
	...
	if not RunService:IsStudio() and Window.Folder and writefile then
		local function normalizePath(p)
			-- Strip trailing slashes; some executors reject them on makefolder/isfolder.
		...
		local function safeMakeFolder(path)
			...
			if not okMk then
				warn("[ ATXUI ] Failed to create folder '" .. tostring(path) .. "': " .. tostring(errMk))
				return false
			end
			return true
		end

		-- Ensure the ATXUI root exists BEFORE any subfolder.
		-- Some executors (Velocity, Wave on PC) silently fail makefolder when the parent is missing.
		safeMakeFolder("ATXUI")
		safeMakeFolder("ATXUI/" .. Window.Folder)
		safeMakeFolder("ATXUI/" .. Window.Folder .. "/assets")
		safeMakeFolder(Window.Folder)
		safeMakeFolder(Window.Folder .. "/assets")
	end
	...
	Window.UIElements.SideBarContainer = New("Frame", {
		Size = UDim2.new(
			...
			Window.SideBarWidth,
			1,
			Window.User.Enabled and -Window.Topbar.Height - 42 - (Window.UIPadding * 2)
		...
		function Window.User:Enable()
			...
			Tween(Window.UIElements.SideBarContainer, 0.25, {
				Size = UDim2.new(
					...
					Window.SideBarWidth,
					1,
					-Window.Topbar.Height - 42 - (Window.UIPadding * 2)
				),
	...
	local BGVideo = typeof(Window.Background) == "string" and string.match(Window.Background, "^video:(.+)")
		or nil
	...
	if typeof(Window.Background) == "string" and BGVideo then
		...
		if string.find(BGVideo, "http") then
			...
			if not isfile(videoPath) then
				...
				if not success then
					warn("[ ATXUI.Window.Background ] Failed to download video: " .. tostring(result))
					return
				end
			end
			...
			if not success then
				warn("[ ATXUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
				return
			end
	...
	Creator.AddSignal(
		...
		function()
			...
			local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X
				/ Config.ATXUI.UIScale
			-- if WindowTitle and WindowAuthor then
			--     LeftWidth = math.max(WindowTitle.TextBounds.X / Config.ATXUI.UIScale, WindowAuthor.TextBounds.X / Config.ATXUI.UIScale)
			-- else
			--     LeftWidth = WindowTitle.TextBounds.X / Config.ATXUI.UIScale
			-- end
			LeftWidth = Window.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X / Config.ATXUI.UIScale
			if Window.Topbar.ButtonsType ~= "Default" then
				LeftWidth = LeftWidth + RightWidth + Window.UIPadding - 4
			end
			-- if WindowIcon then
			--     LeftWidth = LeftWidth + (Window.IconSize / Config.ATXUI.UIScale) + (Window.UIPadding / Config.ATXUI.UIScale) + (4 / Config.ATXUI.UIScale)
			-- end
			Window.UIElements.Main.Main.Topbar.Center.Position =
				UDim2.new(0, LeftWidth + (Window.UIPadding / Config.ATXUI.UIScale), 0.5, 0)
			Window.UIElements.Main.Main.Topbar.Center.Size =
				UDim2.new(1, -LeftWidth - RightWidth - ((Window.UIPadding * 2) / Config.ATXUI.UIScale), 1, 0)
		end
	)

	if Window.Topbar.ButtonsType ~= "Default" then
		Creator.AddSignal(
			Window.UIElements.Main.Main.Topbar.Right:GetPropertyChangedSignal("AbsoluteSize"),
			function()
				Window.UIElements.Main.Main.Topbar.Left.Position = UDim2.new(
					0,
					(Window.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X / Config.ATXUI.UIScale)
						+ Window.UIPadding
						- 4,
					0,
					0
	...
	function Window:CreateTopbarButton(Name, Icon, Callback, LayoutOrder, IconThemed, Color, IconSize)
		...
		local Button = Creator.NewRoundFrame(
			...
			"Squircle",
			{
				...
				AnchorPoint = Vector2.new(0.5, 0.5),
				Position = UDim2.new(0.5, 0, 0.5, 0),
				ImageColor3 = Window.Topbar.ButtonsType ~= "Default" and (Color or Color3.fromHex("#ff3030"))
					or nil,
	...
	if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
		...
		Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
			...
			ImageTransparency = Window.Transparent and Config.ATXUI.TransparencyValue or 0,
		}, {
			BackgroundGradient,
		})
	end
	...
	function Window:SetBackgroundTransparency(v)
		...
		Config.ATXUI.TransparencyValue = rounded
	...
	if Config.ATXUI.UseAcrylic then
	...
	function Window:Open()
		task.spawn(function()
			...
			task.spawn(function()
				...
				Config.ATXUI:ToggleAcrylic(true)
			end)
		end)
	end
	function Window:Close()
		...
		Config.ATXUI:ToggleAcrylic(false)
		...
		task.spawn(function()
			...
			if
				...
				and not Window.IsPC
				and Window.IsOpenButtonEnabled
			then
				Window.OpenButtonMain:Visible(true)
			end
		end)
		...
		function Close:Destroy()
			task.spawn(function()
				...
				task.wait(0.4)
				Config.ATXUI.ScreenGui:Destroy()
				Config.ATXUI.NotificationGui:Destroy()
				Config.ATXUI.DropdownGui:Destroy()
				Config.ATXUI.TooltipGui:Destroy()
	...
	function Window:ToggleTransparency(Value)
		...
		Config.ATXUI.Transparent = Value

		Window.UIElements.Main.Background.ImageTransparency = Value and Config.ATXUI.TransparencyValue or 0
		-- Window.UIElements.Main.Background.ImageLabel.ImageTransparency = Value and Config.ATXUI.TransparencyValue or 0
	...
	function Window:GetUIScale(v)
		return Config.ATXUI.UIScale
	end

	function Window:SetUIScale(v)
		Config.ATXUI.UIScale = v
		Tween(Config.ATXUI.UIScaleObj, 0.2, { Scale = v }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
		return Window
	end
		...
		if typeof(v) == "boolean" then
			...
			if TabModule then
				for _, Container in next, TabModule.Containers do
					Container.ScrollingFrame.UIPadding.PaddingTop =
	...
	function Window:Dialog(DialogConfig)
		...
		local Dialog = DialogModule.Create(false, "Dialog", Window, Config.ATXUI, Window.UIElements.Main.Main)
		...
		local function CheckButtonsOverflow()
			...
			local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.ATXUI.UIScale
			local parentWidth = ButtonsContent.AbsoluteSize.X / Config.ATXUI.UIScale
			...
			if totalWidth > parentWidth then
				...
				if availableSpace > 0 then
					...
					for _, button in ipairs(Buttons) do
						local buttonWidth = button.AbsoluteSize.X / Config.ATXUI.UIScale
	...
	local function stopResizing()
		...
		if resizeEndConnection then
			resizeEndConnection:Disconnect()
	...
	local function startResizing(input)
		if Window.CanResize then
			...
			resizeStartInput = input
			Tween(ResizeHandle.ImageLabel, 0.1, { ImageTransparency = 0.35 }):Play()

			-- Keep a reference to disconnect on End so we don't leak one
			-- connection per resize gesture (and don't wedge it in Creator.Signals).
			if resizeEndConnection then
				resizeEndConnection:Disconnect()
				resizeEndConnection = nil
			end
			resizeEndConnection = input.Changed:Connect(function()
	...
	Creator.AddSignal(UserInputService.InputChanged, function(input)
		if not (isResizing and Window.CanResize) then
			return
		end
		-- Disambiguate simultaneous touches: only follow the input that started the resize.
		if input.UserInputType == Enum.UserInputType.Touch then
			if resizeStartInput == nil or input ~= resizeStartInput then
				return
			end
		...
		local newSize =
			UDim2.new(0, initialSize.X.Offset + delta.X * 2, 0, initialSize.Y.Offset + delta.Y * 2)
		...
		-- Direct assignment avoids 60 tweens/sec fighting each other on mobile touch polling.
