...
function Button.New(Title, Icon, Callback, Variant, Parent, Dialog, FullRounded, Radius)
	...
	local ButtonScale = New("UIScale", { Scale = 1 })
	ButtonScale.Parent = ButtonFrame

	Creator.AddSignal(ButtonFrame.MouseEnter, function()
		Tween(ButtonFrame.Frame, 0.08, { ImageTransparency = 0.95 }):Play()
	end)
	Creator.AddSignal(ButtonFrame.MouseLeave, function()
		Tween(ButtonFrame.Frame, 0.08, { ImageTransparency = 1 }):Play()
		Tween(ButtonScale, 0.2, { Scale = 1 }, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
	end)
	Creator.AddSignal(ButtonFrame.MouseButton1Down, function()
		Tween(ButtonScale, 0.1, { Scale = 0.93 }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
	end)
	Creator.AddSignal(ButtonFrame.MouseButton1Up, function()
		Tween(ButtonScale, 0.3, { Scale = 1 }, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
		if Dialog then --idk
			Dialog:Close()()
		end
		if Callback then
