...
function Element:New(Config)
	...
	Slider.UIElements.SliderContainer = New("Frame", {
		...
		Position = UDim2.new(
			1,
			Slider.IsTextbox and (Config.Window.NewElements and -12 - 4 or 0) or 0,
			0.5,
			0
		),
		...
		if CanCallback then
			...
			then
				if input then
					...
					local inputPosition = isTouch and input.Position.X
					...
					moveconnection = RunService.RenderStepped:Connect(function()
						...
						Value =
							CalculateValue(Slider.Value.Min + delta * (Slider.Value.Max - Slider.Value.Min))

						if Value ~= LastValue then
							Tween(
								Slider.UIElements.SliderIcon.Frame,
								0.05,
								{ Size = UDim2.new(delta, 0, 1, 0) }
							):Play()
							Slider.UIElements.SliderContainer.TextBox.Text = FormatValue(Value)
						...
						then
							...
							if Config.Window.NewElements then
								Tween(Slider.UIElements.SliderIcon.Frame.Thumb, 0.2, {
									...
									Size = UDim2.new(
										0,
										Config.Window.NewElements and (Slider.ThumbSize * 2)
											or (Slider.ThumbSize + 2),
										0,
										Config.Window.NewElements and (Slider.ThumbSize + 4)
											or (Slider.ThumbSize + 2)
									),
	...
	function Slider:SetMax(newMax)
		...
		if currentValue > newMax then
			...
			local newDelta = math.clamp(
				(currentValue - (Slider.Value.Min or 0)) / (newMax - (Slider.Value.Min or 0)),
				0,
				1
			)
