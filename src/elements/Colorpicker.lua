...
function Element:Colorpicker(Config, Window, ATXUI, OnApply)
	local Colorpicker = {
		__type = "Colorpicker",
		Title = Config.Title,
		Desc = Config.Desc,
	...
	local ColorpickerFrame =
		ColorpickerModule.Create(nil, "Dialog", Window, ATXUI, Window.UIElements.Main.Main)
	...
	local Buttons = {
		...
		},
		{
			...
			Callback = function()
				OnApply(
					Color3.fromHSV(Colorpicker.Hue, Colorpicker.Sat, Colorpicker.Vib),
	...
	function Colorpicker:Update(color, transparency, anim)
		if color then
...
function Element:New(Config)
	...
	function Colorpicker:Set(c, t)
		return Colorpicker:Update(c, t, true)
	end

	Creator.AddSignal(Colorpicker.UIElements.Colorpicker.MouseButton1Click, function()
		if CanCallback then
			Element:Colorpicker(Colorpicker, Config.Window, Config.ATXUI, function(color, transparency)
