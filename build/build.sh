...
PKG=$(node -e "const p=require('./package.json');console.log(JSON.stringify({v:p.version||'',d:p.description||'',r:p.repository||'',l:p.license||''}))")
...
echo -e "[ $(date '+%H:%M:%S') ]"
echo -e "${P}[ ✓ ]${R} $PREFIX"
echo -e "${P}[ > ]${R} ATX_UI Build completed successfully"
